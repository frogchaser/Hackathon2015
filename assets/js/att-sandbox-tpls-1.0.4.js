angular.module("att.sandbox", ["att.sandbox.tpls", "att.sandbox.transition","att.sandbox.collapse","att.sandbox.position","att.sandbox.accordion","att.sandbox.alert","att.sandbox.bindHtml","att.sandbox.buttons","att.sandbox.buttonsGroup","att.sandbox.creditCard","att.sandbox.utilities","att.sandbox.datepicker","att.sandbox.facebookLike","att.sandbox.filmstrip","att.sandbox.forms","att.sandbox.modal","att.sandbox.pagination","att.sandbox.promos","att.sandbox.quantitySelect","att.sandbox.rotation","att.sandbox.stepIndicator","att.sandbox.tabs","att.sandbox.toggle","att.sandbox.tooltip","att.sandbox.usagebar","att.sandbox.video"]);
angular.module("att.sandbox.tpls", ["template/accordion/accordion-group.html","template/accordion/accordion.html","template/alert/alert.html","template/buttons/facebook-like-button.html","template/creditCard/creditCardImage.html","template/creditCard/cvcSecurityImg.html","template/datepicker/datepicker.html","template/facebookLike/facebook-like.html","template/filmstrip/attFilmstrip.html","template/filmstrip/attFilmstripContent.html","template/modal/backdrop.html","template/modal/u-filter-window.html","template/modal/u-filter.html","template/modal/window.html","template/pagination/pagination.html","template/promos/promos.html","template/quantitySelect/quantity-select.html","template/rotation/attCarousel.html","template/rotation/attCarouselContent.html","template/stepIndicator/step-indicator.html","template/tabs/tab.html","template/tabs/tabset-titles.html","template/tabs/tabset.html","template/toggle/button-toggle-spanish.html","template/toggle/button-toggle.html","template/tooltip/tooltip-html-unsafe-popup.html","template/tooltip/tooltip-popup-user.html","template/tooltip/tooltip-popup.html","template/tooltip/tooltip-sandbox-style.html","template/usagebar/bar.html","template/usagebar/usage.html"]);
/*!
 * VERSION: 1.7.3
 * DATE: 2014-01-14
...
angular.module('att.sandbox.transition', [])
...
angular.module('att.sandbox.collapse', ['att.sandbox.transition'])
...
angular.module('att.sandbox.position', [])
...
angular.module('att.sandbox.accordion', ['att.sandbox.collapse', 'att.sandbox.position', 'att.sandbox.transition'])
...
.controller('AccordionController',
    ['$scope', '$attrs', 'accordionConfig', '$log',
    ...
    this.scrollTop = function() {
        var scrollTop = angular.isDefined($attrs.scrollTop) ? $scope.$eval($attrs.scrollTop) : accordionConfig.scrollTop;
...
.directive('accordionGroup',
    ['$parse', '$transition', '$scrollTo', '$timeout', '$log', '$position', '$window',
        function($parse, $transition, $scrollTo, $timeout, $log, $position, $window) {
    return {
...
//            }
            ...
            scope.toggle = function() {
                if (!scope.isDisabled) {
                    ...
                    if (scope.isOpen === true && accordionCtrl.scrollTop()) {
                        if (accordionCtrl.isCloseOthers()) {
                            $timeout(function() {
                                $scrollTo(0, $position.offset(element).top);
                            }, 1000);
                        } else {
                            $scrollTo(0, $position.offset(element).top);
                        }
...
angular.module("att.sandbox.alert", []).directive('alert', function () {
  return {
    restrict:'EA',
...
angular.module('att.sandbox.bindHtml', [])
...
angular.module('att.sandbox.buttons', [])
    ...
    .config(['$httpProvider', function($httpProvider) {
        $httpProvider.interceptors.push(['$q', 'progressTracker', function($q, progressTracker) {
            return {
    ...
    .directive('attClickSpin', ['$timeout', '$parse', '$rootScope', 'progressTracker', function($timeout, $parse, $rootScope, progressTracker) {
        return {
            restrict: 'A',
            link: function(scope, elm, attrs) {
                var fn = $parse(attrs.attClickSpin);
                elm.on('click', function(event) {
                    ...
                    $rootScope.loadingTracker = progressTracker({minDuration: 750});//comment this line if not running unit test
    ...
    .directive('attProgressTracker', [function() {
        return {
            restrict: 'EA',
            replace: true,
            template: '<div ng-show="loadingTracker.active()" style="width:100%; text-align:center"><img src="./images/ajax-loader.gif" alt="" /></div>'
        };
    }])
    .directive('buttonSpinner', ['spinnerConfig', function(spinnerConfig) {
        var spinButton = function(state, element, data) {
            ...
            if (state === 'loadingText') {
                ...
                element[attr](data[state]);
                element.removeClass(disabled).removeAttr(disabled, disabled);
            }
        };
        return {
            restrict: 'A',
            ...
            link: function(scope, element, attr) {
                var loadingText = attr.buttonSpinner;
                ...
                var data = {
                    loadingText: validAttr === 'html' ? "<i class=\"cssIcon-spinner\"></i>" + loadingText : loadingText,
                    resetText: element[validAttr]()
                };
                ...
                scope.$on(scope.startEvent, function(e) {
                    spinButton('loading', element, data);
                    scope.$on(scope.stopEvent, function(e) {
                        spinButton('reset', element, data);
                    });
                });
            }
...
angular.module('att.sandbox.buttonsGroup', [])
...
.directive('buttonGroup', ['$log', function($log) {
    return {
        restrict: 'A',
        scope: {maxSelect: "=", ngModelButtonState: '=ngModel'},
        link: function(scope, element, attrs) {
            ...
            element.bind('click', function()
            {
                ...
                if (nSel >= scope.maxSelect)
                {
                    angular.forEach(element.children(), function(chd)
                    {
                        if (chd.className.indexOf('active') < 0) {
                            chd.disabled = true;
                            var child = angular.element(chd);
                            child.append('<span class="hidden-spoken">. Disabled. ' + scope.maxSelect + ' buttons already selected</span>');
                        }
                    });
                }
                else
                {
                    ...
                    {
                        chd.disabled = false;
                        var child1 = angular.element(chd);
...
angular.module('att.sandbox.creditCard', [])
        ...
        .directive('creditCardImage', function() {
            return{
                templateUrl: 'template/creditCard/creditCardImage.html',
                replace: false,
                transclude: false,
                link: function(scope, element, attr)
                {
        ...
        .directive('securityCodeImage', ['$document', function($document) {
                return{
                    templateUrl: 'template/creditCard/cvcSecurityImg.html',
                    replace: false,
                    transclude: false,
                    link: function(scope, element, attr)
                    {
...
angular.module('att.sandbox.utilities', [])
...
angular.module('att.sandbox.datepicker', ['att.sandbox.position', 'att.sandbox.utilities'])
...
.constant('datepickerConfig', {
    ...
    datepickerWatchAttributes: ['min', 'max', 'due', 'from', 'legendIcon', 'legendMessage'],
    datepickerFunctionAttributes: ['disableDates']
})
...
.factory('datepickerService', ['datepickerConfig', 'dateFilter', function (datepickerConfig, dateFilter) {
    var setAttributes = function (attr, elem) {
        if (angular.isDefined(attr) && attr !== null && angular.isDefined(elem) && elem !== null) {
            var attributes = datepickerConfig.datepickerEvalAttributes.concat(datepickerConfig.datepickerWatchAttributes, datepickerConfig.datepickerFunctionAttributes);
            for (var key in attr) {
                var val = attr[key];
...
.controller('DatepickerController', ['$scope', '$attrs', 'dateFilter', '$element', '$position', 'datepickerConfig', function($scope, $attrs, dateFilter, $element, $position, dtConfig) {
    var format = {
    ...
    this.updatePosition = function() {
        ...
        if ($scope.orientation === 'right') {
            $scope.position.left = $scope.position.left - (($element.find('ul').eq(0).prop('offsetWidth') || 290) - $element.find('input').prop('offsetWidth'));
        }
    };
    ...
    var isDisabled = function(date, currentMonthDate) {
        ...
        if (isSelected(date) || isOld(date, currentMonthDate) || isNew(date, currentMonthDate)) {
            return true;
        }
    ...
    function makeDate(date, dayFormat, dayHeaderFormat, isSelected, isDateRange, isOld, isNew, isDisabled, dueDate, pastDue) {
        return {date: date, label: dateFilter(date, dayFormat), header: dateFilter(date, dayHeaderFormat), selected: !!isSelected, dateRange: !!isDateRange, oldMonth: !!isOld, nextMonth: !!isNew, disabled: !!isDisabled, dueDate: !!dueDate, pastDue: !!pastDue};
    }
...
.directive( 'datepickerPopup', ['$parse', '$log', '$timeout', '$document', '$documentBind', '$isElement', function ($parse, $log, $timeout, $document, $documentBind, $isElement) {
    return {
        restrict: 'EA',
        replace: true,
        transclude: true,
        ...
        scope: {},
        require: ['datepickerPopup', 'ngModel'],
        ...
        link: function(scope, element, attrs, ctrls) {
            ...
            function refill(date) {
                ...
                if (selected) {
                    var currentMode = datepickerCtrl.modes[mode], data = currentMode.getVisibleDates(selected);
...
.directive('datepicker', ['$compile', '$log', 'datepickerConfig', 'datepickerService', function($compile, $log, datepickerConfig, datepickerService) {
    return {
        restrict: 'A',
        scope: {
            disableDates: '&'
        ...
        controller: ['$scope', '$element', '$attrs', function(scope, elem, attr) {
            var dateFormatString = angular.isDefined(scope.dateFormat) ? scope.dateFormat : datepickerConfig.dateFormat;
            var selectedDateMessage = '<div class="sr-focus hidden-spoken" tabindex="-1">the date you selected is {{dt | date : \'' + dateFormatString + '\'}}</div>';
            
            elem.removeAttr('datepicker');
            elem.removeAttr('ng-model');
            elem.attr('ng-model', '$parent.dt');
            elem.attr('aria-describedby', 'datepicker');
            elem.attr('format-date', dateFormatString);
...
.directive('formatDate', ['dateFilter', function(dateFilter) {
    ...
    angular.module('att.sandbox.facebookLike', [])
    .constant('facebookConfig', {
        fb: '//www.facebook.com/plugins/like.php?href=',
        fbParam: '&width=75&height=21&colorscheme=light&layout=button_count&action=like&show_faces=false&send=false'
    })
    .directive('attFacebookLike', ['facebookConfig', '$document', '$log', 'trustedFilter', function(facebookConfig, $document, $log, trustedFilter) {
        //This directive will show the like button of facebook 
        return {
            restrict: 'A',
            replace: true,
            templateUrl: 'template/facebookLike/facebook-like.html',
            scope: {
                id: '=' //Id of the iFreame element 
            },
            link: function(scope, element, attrs) {
                if (angular.isDefined($document[0].querySelector("link[rel='canonical']")) && $document[0].querySelector("link[rel='canonical']") !== null) {
                    scope.cannonical = escape(angular.element($document[0].querySelector("link[rel='canonical']")).attr('href'));
                }
                else {
                    if (angular.isUndefined(attrs.source) || attrs.source === null) {
                        scope.cannonical = "";
                        throw  "Cannonical not found";
                    }
                    scope.cannonical = attrs.source;
                }
                scope.source = trustedFilter(facebookConfig.fb + scope.cannonical + facebookConfig.fbParam);
                element.children().css({'background-color': 'black !important'});
            }
        };
    }])
    .filter('trusted', ['$sce', function($sce) {
        // To filter the url resource if not accessable
        return function(url) {
            return $sce.trustAsResourceUrl(url);
        };
    }]);
angular.module('att.sandbox.filmstrip', ['att.sandbox.position'])

.directive('attFilmstrip', [function() {
    return {
        restrict: 'EA',
        transclude: true,
        replace: true,
        ...
        templateUrl: 'template/filmstrip/attFilmstrip.html',
        controller: ['$scope', '$timeout', '$position', '$element', function($scope, $timeout, $position, $element) {
            this.registerElement = function (elem) {
                var unWatch = $scope.$watch(function () {
                    return $position.isElementInViewport($element);
                }, function (value) {
                    if (value) {
                        $timeout(function () {
                            if (elem) {
                                elem.addClass('items-in');
                            }
                            unWatch();
                        });
                    }
...
.directive('attFilmstripContent', [function() {
    return {
        restrict: 'EA',
        transclude: true,
        replace: true,
        scope:{},
        require: '^attFilmstrip',
        templateUrl: 'template/filmstrip/attFilmstripContent.html',
        link: function (scope, elem, attr, ctrl) {
...
angular.module('att.sandbox.forms', [])
.directive('attReset', [function() {
    return {
        require: 'ngModel',
...
//            };
            ...
            var click = function(e) {
                ...
                scope.$apply(function() {
                    if (attrs.attReset)
                    {
                        ctrl.$setViewValue(scope.$eval(attrs.attReset));
                    }
                    else
                    {
                        ctrl.$setViewValue('');
...
.directive("modSelect", ['$timeout', function($timeout) {
    return {
        restrict: 'C',
        link: function(scope, elm, attr, ctrl) {
            if (typeof jQuery !== "undefined" && $.fn.wrapSelect) {
                $.fn.wrapSelect = angular.noop;
            }
            elm.css({
                'opacity': '0',
            ...
            elm.addClass('span12');
            elm.wrap('<span class="selectWrap"></span>');
            var cover = angular.element('<span aria-hidden="true"><i class="cssIcon-chevron-d-w"></i></span>');
            elm.parent().append(cover);
            var set = function() {
                var sel = elm[0] ? elm[0] : elm;
                var selectedText = "";
                var selIndex = sel.selectedIndex;
                if (typeof selIndex !== 'undefined') {
                    selectedText = sel.options[selIndex].text;
                }
                cover.text(selectedText).append('<i class="cssIcon-chevron-d-w"></i>');
            };
            var update = function() {
                $timeout(set, 100);
            };
            if (elm.attr("ng-model")) {
                scope.$watch(elm.attr("ng-model"), function(newVal, oldVal) {
                    update();
                });
            }
...
angular.module('att.sandbox.modal', ['att.sandbox.position','att.sandbox.transition','att.sandbox.utilities'])
 ...
 */
  ...
  .directive('modalBody', ['$timeout','$position','$log','$window', function ($timeout,$position,$log,$window) {
    return {
      restrict: 'AC',
      scope: {
        index: '@'
      ...
      replace: true,
      link: function (scope, element, attrs) {
          //fix the modal-body height according to the screen size.
          var window = angular.element($window);
          scope.setModalHeight= function(){
          var modalHeight,modalFooter,windowHeight,bodyHeight;     
          var modalElements=element.parent().children(); 
          if(modalElements.eq(0).hasClass('modal-header')){
              modalHeight=$position.position(modalElements.eq(0)).height;
              if(modalElements.eq(2).hasClass('modal-footer')){
                  modalFooter=$position.position(modalElements.eq(2)).height;
              }else{
                  modalFooter=0;
              }
          }else{
              modalHeight=0;
              if(modalElements.lenght===1){
                  modalFooter=0;
              }
              else{
                  modalFooter=$position.position(modalElements.eq(1)).height;
              }
          }
           windowHeight=$window.innerHeight;
          bodyHeight=windowHeight-(modalFooter+modalHeight)+ 'px' ;
  ...
  .factory('$modalStack', ['$document', '$compile', '$rootScope', '$$stackedMap', '$log',
    function ($document, $compile, $rootScope, $$stackedMap, $log) {

      var backdropjqLiteEl, backdropDomEl;
      var backdropScope = $rootScope.$new(true);
      var body = $document.find('body').eq(0);
      var html=$document.find('html').eq(0);
      var scrollToHeight=null;
      ...
      function removeModalWindow(modalInstance) {
         ...
         if(angular.isObject(scrollToHeight)){
         scrollToHeight.focus();
       }
      ...
      $modalStack.open = function (modalInstance, modal) {
        ...
        var angularDomEl = angular.element('<div modal-window tabindex="0"></div>');
        angularDomEl.bind("focus", function(ev) {
            $log.log("am creating focus on modal", ev);
            scrollToHeight=ev.relatedTarget;
        });
...
.directive("utilityFilter", ["$modal", "$log",'$scrollTo', function($modal, $log,$scrollTo) {
        return {
            ...
            require: 'ngModel',
            templateUrl: 'template/modal/u-filter.html',
            link: function(scope, element, attribute, ctrl) {
                ...
                if (angular.isDefined(scope.utilityFilter)) {
                    ...
                    scope.templateUrl = 'template/modal/u-filter-window.html';
                }
                element.bind('click', function(ev) {
                    var currentPosition= ev.pageY-ev.clientY;
                    $modal.open({
...
angular.module('att.sandbox.pagination', [])
.directive("attPagination", ['$timeout','$log','$rootScope', function($timeout,$log,$rootScope) {
    return {
        restrict: 'EA',
        replace:'true',
        transclude:'true',
     ...
     templateUrl:'template/pagination/pagination.html',
        ...
        link: function(scope, element, attribute) { 
            if(angular.isUndefined(scope.noOfResult)){
                scope.noOfResult=10;
            }
            if(angular.isUndefined(scope.searchResult)){
                throw "SearchResult not provided as a directive attribute";
            }
            scope.showQuantity=parseInt(scope.noOfResult,10);
            scope.totalResult=parseInt(scope.searchResult.length,10);
            scope.resultLeft=parseInt(scope.totalResult,10)-parseInt(scope.showQuantity,10);
            element.find('button').bind('click', function() { 
                   // $rootScope.$broadcast("startButtonSpinner");
                   scope.$watch("resultLeft",function(oldValue,newValue){
                       if(newValue<=0){
                            element.find('button').css({'display':'none'});
                           //element.find('button').attr({'disabled': 'disabled'});
                            //element.find('button').text("no more results");
                       }
                   }) ;
                    if (scope.showQuantity > scope.totalResult) {
                        element.find('button').attr({'disabled': 'disabled'});
                    }
                    else {
                        scope.showQuantity = parseInt(scope.showQuantity,10) + parseInt(scope.noOfResult,10);
                        scope.resultLeft=parseInt(scope.totalResult,10)-parseInt(scope.showQuantity,10);
                        scope.$apply();
                    }                                       
                    
                    // $rootScope.$broadcast("stopButtonSpinner");
                    setTimeout(function() {
                     //   $rootScope.$broadcast("stopButtonSpinner");
                     $log.log("time out");
                    }, 1000);
...
angular.module('att.sandbox.promos', ['ngResource'])
        .constant('personalizationURIs',
        {
            service: {
                promoUri: "",
                promoStubUri: "stub/promos/promos.json",
                promoStubUri2: "stub/promos/promos2.json"
            },
            imageService: {
                imageUri: "",
                imageStubUri: "images/"
        ...
        .constant("personalizationConfig",
        {
            "useStubs": true,
            "imageServiceConfig": {
                "useStubs": true
            ...
            "templateRequest": {
                ...
                "targeterInfo": {
                    "targeterPath": "Wireless/DSS/Landing1"
                },
                "accountInfo": {
                    "accountType": "WIRELESS",
                    "wireless": {
                        "wirelessSubscriberNumber": "",
       ...
       .service(
        ...
        [
            ...
            function(
                ...
                var _fetchPromosService = $resource(personalizationURIs.service.promoUri, {},
                        {fetchPromosService: {method: 'POST', headers: personalizationConfig.headers}});
                var _fetchPromosStubService = $resource(personalizationURIs.service.promoStubUri);
                var _fetchPromosStubService2 = $resource(personalizationURIs.service.promoStubUri2);
                var promoMap = {};
                var promos = {accordions: [], dashboards: []};

                var parsePromotions = function(data) {
                    var imageLinkPrefix;
                    if (personalizationConfig.imageServiceConfig.useStubs) {
                        imageLinkPrefix = personalizationURIs.imageService.imageStubUri;
                    } else {
                        imageLinkPrefix = personalizationURIs.imageService.imageUri;
                    }

                    var promo;
                    var link;
                    for (var ctr1 in data.promotions) {
                        promo = data.promotions[ctr1];
                        if (promo.SlotType == 'DSS') {
                            if (promo.LayoutType == 'ImageOnly') {
                                for (var ctr2 in promo.Links) {
                                    link = promo.Links[ctr2];
                                    //GraphicOnly means LinkUrl would be internal
                                    link.LinkGraphic = imageLinkPrefix + link.LinkGraphic;
                                }
                                promos.dashboards.push(promo);
                            } else {
                                promos.accordions.push(promo);
                            }
                        }
                    }
                ...
                //pageId parameter is for future use
                var _getPromos = function(pageId) {
                    log.info("promoService :: getPromos : pageId : " + pageId);
                    var deferred = q.defer();
                    var deferred2 = q.defer();

                    var promoRequest = angular.copy(personalizationConfig.templateRequest);
                    try {
                        if (promoMap && promoMap[pageId] && promoMap[pageId].dashboards && promoMap[pageId].dashboards.length > 0) {
                            promos = {accordions: [], dashboards: []};
                            promoMap[pageId] = promos;
                        }
                    } catch (e) {
                        log.error("Exception : promoMap: " + e.toString());
                    }

                    var success = function(data) {
                        log.info("promoService::received response from service1 :: " + angular.toJson(data));
                        promoMap[pageId] = parsePromotions(data);
                        deferred.resolve(promoMap[pageId]);
                    };
                    var success2 = function(data) {
                        log.info("promoService::received response from service2 :: " + angular.toJson(data));
                        promoMap[pageId] = parsePromotions(data);
                        deferred2.resolve(promoMap[pageId]);
                    };
                    var error = function(e) {
                        log.info("_fetchPromosService :: exception " + e.toString());
                    };

                    log.info("promoService::getPromos:request created " + angular.toJson(promoRequest));
                    promoMap[pageId] = promos;

                    if (personalizationConfig.useStubs) {
                        log.info("promoService::going to make call using stub");
                        _fetchPromosStubService.get({}, success, error);
                        _fetchPromosStubService2.get({}, success2, error);
                    } else {
                        log.info("promoService:: promo Request1 = " + angular.toJson(promoRequest));
                        _fetchPromosService.fetchPromosService(promoRequest, success, error);
                        var promoRequest1 = angular.copy(promoRequest);

                        promoRequest1.targeterInfo.targeterPath = "Wireless/DSS/Landing2";
                        log.info("promoService:: promo Request2 =" + angular.toJson(promoRequest1));
                        _fetchPromosService.fetchPromosService(promoRequest1, success2, error);
                    }
                    return deferred2.promise;
                };

                return {
                    getPromos: _getPromos
                };
            }
        ])
      .directive('attPromos', [
        'PromoService',
        '$timeout',
        function(PromoService,$timeout) {

            return {
                templateUrl: 'template/promos/promos.html',
                restrict: 'EA',
                controller: ['$scope', function($scope) {
                    $scope.interval = 2000;
                    $scope.noPause = true;
                    $scope.attPromos = {};
                    PromoService.getPromos().then(
                            function(data) {
                                $scope.attPromos.images = data.dashboards;
                            });
                }],
                link : function (scope,element,attrs){
                    ...
                    function doSlide(){
                        ...
                        next = function() {
                            if(scope.currentIndex < scope.attPromos.images.length - 1){
                                 scope.currentIndex++;
                            }
                            else{
                                 scope.currentIndex = 0;
                        ...
                        scope.$watch('currentIndex', function() {
                            scope.attPromos.images.forEach(function(image) {
                                   image.visible = false;
                            });
                            scope.attPromos.images[scope.currentIndex].visible = true;
                        });
                        var timer;
                 ...
                 scope.$watch('attPromos.images',function(newValue, oldValue){
...
angular.module('att.sandbox.quantitySelect', [])

.directive('counter',['$log','$parse', function($log,$parse) {
        return {
            restrict: 'A',
            require:'?ngModel',
            scope:
            ...
            replace:true,
            transclude:true,
            templateUrl:'template/quantitySelect/quantity-select.html',
            link: function( scope , element , attributes ) {
                ...
                var min,max,step;
                if (attributes.min) {
                scope.$parent.$watch($parse(attributes.min), function(value) {
                    if(value===0){
                        min=parseInt(value,10);
                    }
                    else{
                        min=value ? parseInt(value,10):null;
                    }
                  
                });
                }
                if (attributes.max) {
                scope.$parent.$watch($parse(attributes.max), function(value) {
                   max=value ? parseInt(value,10):null;
                   
                });
                }
                if (attributes.step) {
                scope.$parent.$watch($parse(attributes.step), function(value) {
                   step=value ? parseInt(value,10):null;
                  
                });
                }
                
                var relation = angular.isUndefined(attributes.relation) ? null : String(attributes.relation);
                ...
                scope.readonly = angular.isUndefined(attributes.editable) ? true : false;
                  element.find('input').bind('keyup',function(){
                            if (isNaN(this.value)) {
                                ...
                                scope.value = this.value ? parseInt(this.value, 10) : parseInt(-1, 10);
                                scope.$apply();
                            }
                });
     ...
     var checkValue=function(){
                    if(scope.value===min){
                        element.find('button').eq(0).attr('disabled','disabled');
                       element.find('button').eq(1).removeAttr('disabled');
                   }
                   else if(scope.value===max){
                       element.find('button').eq(0).removeAttr('disabled');
                       element.find('button').eq(1).attr('disabled','disabled');                                           
                   }
                   else {
                      element.find('button').eq(0).removeAttr('disabled');
                       element.find('button').eq(1).removeAttr('disabled');
                ...
                scope.minus = function() {
                     if((scope.value-step)===min||(scope.value===min)){
                       var buttonType=element.find('button');
                       element.find('button').eq(0).attr('disabled','disabled');
                       element.find('button').eq(1).removeAttr('disabled');
                 ...
                 */
                scope.plus = function() {
                    if((scope.value+step)===max||(scope.value===max)){
                       var buttonType=element.find('button');
                       element.find('button').eq(0).removeAttr('disabled');
                       element.find('button').eq(1).attr('disabled','disabled');
        ...
        .directive('quantitySelect', ['$compile', '$log', '$document', function($compile, $log, $document) {
                return{
                    restrict: 'A',
                     require:'?ngModel',
                    scope: {
                    ...
                    link: function(scope, element, attribute,ngModelCtrl) {
                        //scope.value = parseInt(attribute.value, 10);
                        scope.readonly = angular.isUndefined(attribute.editable) ? true : false;
                        element.removeAttr('quantity-select');
                        ...
                        var popup = angular.element('<span counter ng-model="value">' + element.prop('outerHTML') + '</span>');
                        popup.attr({
                            'min': 'min',
                            'max': 'max',
                            'step': 'step'
                        ...
                        var popupEL = $compile(popup)(scope);
                        element.replaceWith(popupEL);
                    }
                };
            }]);
angular.module('att.sandbox.rotation', ['att.sandbox.transition', 'angular-gestures'])
...
.directive('attCarousel', [function() {
    return {
        ...
        require: 'attCarousel',
        templateUrl: 'template/rotation/attCarousel.html',
        scope: {
            interval: '=',
            noTransition: '=',
            noPause: '='
...
.directive('attCarouselContent', function() {
    return {
        require: '^attCarousel',
        restrict: 'EA',
        transclude: true,
        replace: true,
        templateUrl: 'template/rotation/attCarouselContent.html',
        scope: {
            active: '=?'
        },
...
angular.module("att.sandbox.stepIndicator", [])
        .directive("attStepIndicator", ['$log', '$window', '$timeout', function($log, $window, $timeout) {
                return {
                    ...
                    scope: {
                        steps:'=',     //takes the number os steps os Step-Indicator currently max=4
                        stepsDone:'=', //takes how many steps completed .for initial stage can be skipped
                        stepName:'@?', //takes the optional name for the current step
                        stepDesc:'@?'  //takes the optional description for the current step
                    },
                   templateUrl:'template/stepIndicator/step-indicator.html',
                    link: function(scope, element, attrs)
                    {
                        if (angular.isUndefined(scope.steps)) {
                            throw "Missing the steps attribute on the attstepIndicator directive.";
                        }
                        scope.numlength = [];
                        //For initial stage css building    
                        if (angular.isUndefined(scope.stepsDone)) {
                            var j;
                            for ( j = 0; j != scope.steps; j++) {
                                scope.numlength[j] = {id: j + 1};
                                angular.extend(scope.numlength[j], {text: "Remaining Step " + (j + 1), cls: "step"});
                            }
                                scope.numlength[j] = {id: j + 1};
                                angular.extend(scope.numlength[j], {text: "Finished ", cls: "step last"});
                        }
                        else {
                            var i;
                            for (i = 0; i != scope.steps; i++)
                            {
                                scope.numlength[i] = {id: i + 1};
                                if (i < scope.stepsDone) {
                                    angular.extend(scope.numlength[i], {text: "Completed Step " + (i + 1), cls: "step step-done"});
                                }
                                if (i > scope.stepsDone) {
                                    angular.extend(scope.numlength[i], {text: "Remaining Step " + (i + 1), cls: "step"});
                                }
                                if (i == scope.stepsDone) {
                                    angular.extend(scope.numlength[i], {text: "Currently on Step " + (i + 1), cls: "step step-on"});
                                }
                            }
                            if(scope.steps===scope.stepsDone){
                             scope.numlength[i] = {id: i + 1};
                             angular.extend(scope.numlength[i], {text: "Finished ", cls: "step step-done last"});
                             //class="last" is required refer JQ Sandbox
                        }else{
                             scope.numlength[i] = {id: i + 1};
                             angular.extend(scope.numlength[i], {text: "Finished ", cls: "step last"});
                        }
                    }
                    }
                };
        }])
        .directive('indicatorSpacer', [function () {
            return {
...
/**
 * @ngdoc overview
 * @name att.sandbox.tabs
 *
 * @description
 * AngularJS version of the tabs directive.
 */

angular.module('att.sandbox.tabs', [])
...
/**
 * @ngdoc directive
 * @name att.sandbox.tabs.directive:tabset
 * @restrict EA
 *
 * @description
...
/**
 * @ngdoc directive
 * @name att.sandbox.tabs.directive:tab
 * @restrict EA
 *
 * @param {string=} heading The visible heading, or title, of the tab. Set HTML headings with {@link att.sandbox.tabs.directive:tabHeading tabHeading}.
 * @param {string=} select An expression to evaluate when the tab is selected.
 * @param {boolean=} active A binding, telling whether or not this tab is selected.
 * @param {boolean=} disabled A binding, telling whether or not this tab is disabled.
 *
 * @description
 * Creates a tab with a heading and content. Must be placed within a {@link att.sandbox.tabs.directive:tabset tabset}.
 *
 * @example
<example module="att.sandbox">
  <file name="index.html">
...
/**
 * @ngdoc directive
 * @name att.sandbox.tabs.directive:tabHeading
 * @restrict EA
 *
 * @description
 * Creates an HTML heading for a {@link att.sandbox.tabs.directive:tab tab}. Must be placed as a child of a tab element.
 *
 * @example
<example module="att.sandbox">
  <file name="index.html">
...
angular.module('att.sandbox.toggle', [])
        .directive('toggleButtonWrapper', ['$compile', '$log', '$document', function($compile, $log, $document) {
                return{
                    restrict: 'AE',
                    scope: {
                    ...
                    transclude: true,
                    replace: true,
                    templateUrl: 'template/toggle/button-toggle.html',
                    link: function(scope, element, attribute) {
                        scope.SlideOn = function()
                        {
                            element.find('input').next('.btn-slider-on').next('.btn-slider').css({'margin-left': '34px'});
                            element.find('input').next('.btn-slider-on').css({'background-color': '#438616'});
                            element.find('input').next('.btn-slider-on').next('.btn-slider').next('.btn-slider-off').css({'background-color': '#438616'});
                        };

                        scope.SlideOff = function()
                        {
                            element.find('input').next('.btn-slider-on').next('.btn-slider').css({'margin-left': '0px'});
                            element.find('input').next('.btn-slider-on').css({'background-color': '#666666'});
                            element.find('input').next('.btn-slider-on').next('.btn-slider').next('.btn-slider-off').css({'background-color': '#666666'});
                        };
                        //checking that id should be there for toggle button
                        if (angular.isUndefined(element.find('input').attr('id'))) {
                            throw "Missing id value of the toggle button.";
                        }
                        if (angular.isDefined(element.find('input').attr('id'))) {
                            var idForButton = element.find('input').attr('id');
                            //element.find('label').removeAttr('for');
                            element.find('label').attr({'for': idForButton});
                        }
                        //setting the initial state of the button using the checkbox property
                        if (element.find('input').attr('checked')) {
                            scope.ngModelButtonState = 'On';
                            scope.SlideOn();
                        }
                        else {
                            scope.ngModelButtonState = 'Off';
                            scope.SlideOff();
                        }

                        //chengeing the button state with click                         
        ...
        .directive('toggleButton', ['$compile', '$log', '$document', function($compile, $log, $document) {
                //the actual directive to be used for showing the toggle button .
                return{
                    restrict: 'A',
                    require: '?ngModel',
                    scope: {
                        ngModelOnButton: '=ngModel'
                    },
                    transclude: false,
                    replace: false,                    
                    link: function(scope, element, attribute, ngModel) {
                        ...
                        if(attribute.on)
                        {
                            scope.onValue = attribute.on;                        
                        }                        
                        if(attribute.off)
                        {
                            scope.offValue = attribute.off;
                        }                        

                        if(attribute.typespanish === "")
                        {
                        ...
                        var buttonTemplate = '<span class="btn btn-slider-on"><span ng-show="!isSpanish">{{onValue}}</span><span class="hidden-spoken">{{onValue}}</span><i class="activo" ng-show="isSpanish">activo</i></span>' +
                                '<span class="btn btn-alt btn-slider"></span>' +
                                '<span class="btn btn-slider-off pull-right"><span ng-show="!isSpanish">{{offValue}}</span><span class="hidden-spoken">{{offValue}}</span><i class="inactivo" ng-show="isSpanish">inactivo</i></span>';
                        scope.count = 0;
                        element.removeAttr('toggle-button');
                        element.removeAttr('ng-model');
                        var toggleButton = angular.element('<div toggle-button-wrapper ng-model="ngModelOnButton">' + element.prop('outerHTML') + buttonTemplate + '</div>');
                        var toggleButtonEL = $compile(toggleButton)(scope);
                        element.replaceWith(toggleButtonEL);
                        
                        
                        
                        
                        scope.$watch('ngModelOnButton', function() {
                            // watch on the button state 
                        });
                    }
                };
            }])
        
        .directive('toggleButtonSpanishWrapper', ['$compile', '$log', '$document', function($compile, $log, $document) {
                return{
                    restrict: 'AE',
                    scope: {
                        buttonState: '=ngModel',
                    ...
                    transclude: true,
                    replace: true,
                    templateUrl: 'template/toggle/button-toggle-spanish.html',
                    link: function(scope, element, attribute) {
                        scope.SlideOn = function()
                        {
                            element.find('input').next('.btn-slider-on').next('.btn-slider').css({'margin-left': '34px'});
        ...
        .directive('toggleButtonSpanish', ['$compile', '$log', '$document', function($compile, $log, $document) {
                return{
                    restrict: 'A',
                    require: '?ngModel',
                    scope: {
                        ngModelButtonSpanish: '=ngModel'
                    },
                    transclude: false,
                    replace: false,
                    link: function(scope, element, attribute, ngModel) {
                        var buttonTemplate = '<span class="btn btn-large btn-slider-on">' +
                                '<span class="hidden-spoken">seleccione para hacer </span><i class="activo">activo</i></span>' +
                                '<span class="btn btn-large btn-alt btn-slider"></span>' +
                                '<span class="btn btn-large btn-slider-off"><span class="hidden-spoken">seleccione para hacer </span><i class="inactivo">inactivo</i></span>';
                        element.removeAttr('toggle-button-spanish');
                        element.removeAttr('ng-model');
                        var toggleButton = angular.element('<div toggle-button-spanish-wrapper ng-model="ngModelButtonSpanish">' + element.prop('outerHTML') + buttonTemplate + '</div>');
                        var toggleButtonEL = $compile(toggleButton)(scope);
                        element.replaceWith(toggleButtonEL);
                        scope.$watch('ngModelButtonSpanish', function() {
                            // watch on the model to add change functionality
                        });
                    }
                };
            }]);
 ...
 */
angular.module('att.sandbox.tooltip', ['att.sandbox.position', 'att.sandbox.bindHtml'])
        ...
        .provider('$tooltip', function() {
             ...
             * application.
             *
             *   var app = angular.module( 'App', ['att.sandbox.tooltip'], function( $tooltipProvider ) {
                        ...
                        return {
                            ...
                            link: function link(scope, element, attrs) {
                                ...
                                element.bind('mouseleave', function(){
                                    element.attr("title", angular.element(scope.tt_content.trim()).text());
                                });
                                ...
                                function show() {
                                    ...
                                    if (appendToBody) {
                                        ...
                                        if(element.next().hasClass("reset-field"))
                                        {
                                            element.next().after(tooltip);
                                        }
                                        else
                                        {
        ...
        .directive("tooltipSandbox", ["$log", "$document", function($log, $document) {
                return{
                    restrict: 'EA',
                    scope: {
                        sandboxStyle: '@',
                    ...
                    replace: true,
                    transclude: true,
                    templateUrl: 'template/tooltip/tooltip-sandbox-style.html',
                    link: function(scope, element, attr) {
                        scope.isActive = false;
                        element.bind("click", function(ev) {
                            ev.preventDefault();
                            var tooltipClass = element.find("a");
                            if (scope.isActive) {
                                tooltipClass.removeClass("active");
                                scope.isActive = false;
                                scope.$apply();
                            } else {
                                tooltipClass.addClass("active");
                                scope.isActive = true;
                                scope.$apply();
                            }
                        });
                        // close all tooltips if active clicking something else.
                        var window = angular.element($document);
                        window.bind("click", function(ev) {
                            if (element.find("a")[0] !== ev.target) {
        ...
        .directive("tooltipUserinitiated", ["$log", "$document", function($log, $document) {
                return{
                    restrict: 'EA',
                    scope: {
                        role: "@"
                    ...
                    replace: true,
                    transclude: true,
                    templateUrl: 'template/tooltip/tooltip-popup-user.html',
                    link: function(scope, element, attr) {
                         scope.isActive = false;
                        element.find('button').bind("click", function(ev) {
                            ev.preventDefault();
                            if (scope.isActive) {
                                element.find("button").removeClass("active");
                                scope.isActive = false;
                       ...
                       var window = angular.element($document);
                        window.bind("click", function(ev) {
                        if ((element.find("button")[0] !== ev.target)&&(element.parent().find('input')[0]!==ev.target)) {
                                element.find("button").removeClass("active");
                                 scope.isActive = false;
                                scope.$apply();
                            }
...
angular.module('att.sandbox.usagebar', ['att.sandbox.transition'])

.constant('progressConfig', {
  animate: true,
  autoType: false,
  stackedTypes: ['success', 'warning', 'danger']
})
...
.directive('usage', function() {
    return {
        ...
        templateUrl: 'template/usagebar/usage.html',
...
.directive('progressbar', ['$transition', function($transition) {
    return {
        ...
        templateUrl: 'template/usagebar/bar.html',
        link: function(scope, element) {
            scope.$watch('width', function(value) {
                if (scope.animate) {
                    element.css('width', scope.old + '%');
                    element.append('<div class="hidden-spoken">'+scope.old+'%</div>');
                    $transition(element, {width: value + '%'});
                } else {
                    element.css('width', value + '%');
                     element.append('<div class="hidden-spoken">'+value+'%</div>');
                }
            });
        }
    };
}]);
angular.module('att.sandbox.video', [])

.directive('attVideo', ['$document', '$log', function ($document, $log) {
    var linkFunction = function (scope, elem, attr, ctrl) {
        if (typeof gvp === "object") {
            gvp.getMetaData([scope.videoId], function (gvpData) {
                var videodata = gvpData.gvpResponse.videoData[0];
                var title = videodata.title;
                var thumbnailLarger = videodata.poster;
                var thumbnailSmaller = videodata.thumbFilePath;
                var videoContainer = elem.css({
                    height: scope.height + 'px',
                    width: scope.width + 'px'
                });
                var videoThumb = angular.element('<img />');
                videoThumb.css({
                    height: scope.height + 'px',
                    width: scope.width + 'px'
                })
                .attr({
                    src: ((scope.width > 110) ? thumbnailLarger : thumbnailSmaller),
                    alt: title
                });
                videoContainer.append(videoThumb);

                gvp.injectVideoLink(scope.videoId, elem.attr('id'));
            });
        } else {
            $log.error("GVP.js is not loaded.");
        }
    };
    ...
    return {
        ...
        compile: function (elem, attr) {
            var loadJsFile = function (url) {
                ...
                fileScript.setAttribute("src", url);
            ...
            if (typeof gvp !== "object") {
                loadJsFile('//www.att.com/media/gvp/GVP.js');
            }
...
angular.module("template/accordion/accordion-group.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/accordion/accordion-group.html",
    "<div class=\"accordion-group tabpanel\">\n" +
    "    <h3 id=\"tab{{index}}\" class=\"toggle-header accordion-heading accordion-toggle\" aria-selected=\"{{focused}}\" aria-controls=\"panel{{index}}\" ng-class=\"{focus: focused, selected: focused}\" aria-expanded=\"{{isOpen}}\" role=\"tab\" tabindex=\"0\" ng-click=\"toggle()\" accordion-transclude=\"heading\">\n" +
    "        <div ng-switch on=\"isOpen\">\n" +
    "            <i class=\"cssIcon-toggle-plus\" ng-switch-when=\"false\">collapsed</i>\n" +
    "            <i class=\"cssIcon-toggle-minus\" ng-switch-when=\"true\">expanded</i>\n" +
    "        </div>\n" +
    "        <span ng-class=\"{'text-muted': isDisabled}\">{{heading}}</span>\n" +
    "    </h3>\n" +
    "    <div id=\"panel{{index}}\" aria-labelledby=\"tab{{index}}\" aria-hidden=\"{{!isOpen}}\" role=\"tabpanel\" collapse=\"!isOpen\" class=\"accordion-body\" ng-transclude>\n" +
    "    </div>\n" +
    "</div>\n" +
    "");
}]);
...
angular.module("template/accordion/accordion.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/accordion/accordion.html",
    "<div class=\"accordion\" role=\"tablist\" ng-transclude ></div>");
}]);

angular.module("template/alert/alert.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/alert/alert.html",
    "<div class='alert' ng-class='type && \"alert-\" + type'>\n" +
    "    <button ng-show='closeable' type='button' class='close' ng-click='close()'>Dismiss</button>\n" +
    "        <i  ng-if=\"showicon\" ng-class='type && \"cssIcon-\" + type'></i>\n" +
    "        <div ng-transclude style=\"width:260px !important\">\n" +
    "            \n" +
    "        </div>\n" +
    "</div>\n" +
    "");
...
angular.module("template/buttons/facebook-like-button.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/buttons/facebook-like-button.html",
    "<span class=\"btn-facebook-like\">\n" +
    "    <iframe id=\"{{id}}\" src=\"{{source}}\" \n" +
...
angular.module("template/creditCard/creditCardImage.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/creditCard/creditCardImage.html",
...
angular.module("template/creditCard/cvcSecurityImg.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/creditCard/cvcSecurityImg.html",
    "<div>\n" +
...
angular.module("template/datepicker/datepicker.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/datepicker/datepicker.html",
    "<div>\n" +
    "    <span class=\"cssIcon-calendar\" style=\"z-index: 999\" ng-transclude></span>\n" +
    "    <ul id=\"datepicker\" class=\"datepicker datepicker-dropdown dropdown-menu datepicker-orient-top\" ng-class=\"{'datepicker-orient-left': orientation === 'left', 'datepicker-orient-right': orientation === 'right'}\" ng-style=\"{display: (isOpen && 'block') || 'none', top: position.top + 'px', left: position.left + 'px'}\" aria-hidden=\"false\" role=\"dialog\" tabindex=\"-1\">\n" +
    "        <div class=\"datepicker-days\" style=\"display: block;\">\n" +
    "            <table class=\"table-condensed\">\n" +
    "                <thead>\n" +
    "                    <tr>\n" +
    "                        <th id=\"prev\" tabindex=\"0\" ng-class=\"{'prev': !disablePrev}\" ng-style=\"{visibility: visibilityPrev}\" ng-click=\"!disablePrev && move(-1)\"><span class=\"hidden-spoken\">Previous Month</span></th>\n" +
    "                        <th id=\"month\" tabindex=\"-1\" class=\"datepicker-switch\" colspan=\"{{rows[0].length - 2}}\">{{title}}</th>\n" +
    "                        <th id=\"next\" tabindex=\"0\" ng-class=\"{'next': !disableNext}\" ng-style=\"{visibility: visibilityNext}\" ng-click=\"!disableNext && move(1)\"><span class=\"hidden-spoken\">Next Month</span></th>\n" +
    "                    </tr>\n" +
    "                    <tr ng-show=\"labels.length > 0\">\n" +
    "                        <th id=\"{{label.post}}\" class=\"dow\" ng-repeat=\"label in labels\"><span aria-hidden=\"true\">{{label.pre}}</span><span class=\"hidden-spoken\">{{label.post}}</span></th>\n" +
    "                    </tr>\n" +
    "                </thead>\n" +
    "                <tbody>\n" +
    "                    <tr ng-repeat=\"row in rows\">\n" +
    "                        <td headers=\"{{dt.header}}\" tabindex=\"{{(!dt.disabled && 0) || -1}}\" aria-hidden=\"{{(dt.disabled && true) || false}}\" ng-repeat=\"dt in row\" class=\"day\" ng-class=\"{'active': ((dt.selected || dt.dateRange) && !dt.oldMonth && !dt.nextMonth), 'old': dt.oldMonth, 'new': dt.nextMonth, 'disabled': dt.disabled, 'due-date': dt.dueDate, 'late-fee': dt.pastDue}\" title=\"{{((dt.dueDate || dt.pastDue) && legendMessage) || ''}}\" ng-click=\"!dt.disabled && select(dt.date)\">{{dt.label}}<span class=\"hidden-spoken\">{{(dt.dueDate && 'Bill Due Date') || ''}}</span><span class=\"hidden-spoken\">{{((dt.dueDate || dt.pastDue) && legendMessage) || ''}}</span></td>\n" +
    "                    </tr>\n" +
    "                </tbody>\n" +
    "                <tfoot ng-show=\"legendIcon && legendMessage\">\n" +
    "                    <tr>\n" +
    "                        <th id=\"legend\" colspan=\"7\" class=\"text-left\">\n" +
    "                            <ul class=\"calendar-legend\">\n" +
    "                                <li>\n" +
    "                                    <i ng-class=\"legendIcon\"></i> {{legendMessage}}\n" +
    "                                </li>\n" +
    "                            </ul>\n" +
    "                        </th>\n" +
    "                    </tr>                \n" +
...
angular.module("template/facebookLike/facebook-like.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/facebookLike/facebook-like.html",
    "<span class=\"btn-facebook-like\">\n" +
    "    <iframe id=\"{{id}}\" src=\"{{source}}\" \n" +
    "            scrolling=\"no\" frameborder=\"0\" style=\"border:none; overflow:hidden; width:auto; height:21px;\" \n" +
    "            allowtransparency=\"true\">     \n" +
...
angular.module("template/filmstrip/attFilmstrip.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/filmstrip/attFilmstrip.html",
    "<div class=\"filmstrip\" ng-class=\"{'filmstrip-icons': (filmstripType==='icon'), 'filmstrip-videos-small': (filmstripType==='videoSmall'), 'filmstrip-videos-large': (filmstripType==='videoLarge')}\">\n" +
    "    <h3 class=\"filmstrip-header\" ng-bind=\"filmstripHeading\"></h3>\n" +
    "    <ul class=\"filmstrip-strip\" ng-class=\"{'row-fluid-nowrap list no-arrow': (filmstripType=='videoSmall') || (filmstripType=='videoLarge')}\" ng-transclude></ul>\n" +
    "</div>");
}]);

angular.module("template/filmstrip/attFilmstripContent.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/filmstrip/attFilmstripContent.html",
    "<li ng-transclude></li>");
}]);
...
angular.module("template/modal/u-filter-window.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/modal/u-filter-window.html",
    "<div class=\"modal utility-bg fade in\" role=\"dialog\" aria-labelledby=\"myUtilModal\">\n" +
    "    <div class=\"modal-body\">\n" +
    "        <form class=\"filterTank\">\n" +
    "            <fieldset ng-repeat=\"option in options\" >\n" +
    "                <legend>{{option.fieldset}}</legend>\n" +
    "                <label ng-if=\"option.inputType === 'radio'\" ng-repeat=\"opt in option.labels\" for=\"{{opt.for}}\" class=\"radio\">\n" +
    "                    <input type=\"radio\" ng-model=\"option.selValue\" name=\"{{opt.name}}\" id=\"{{opt.for}}\" value=\"{{opt.value}}\" />\n" +
    "                    <i class=\"skin\"></i>\n" +
    "                    <span>{{opt.text}}</span>\n" +
    "                </label>\n" +
    "                <div ng-if=\"option.inputType === 'checkbox'\" ng-repeat=\"opt in option.checkboxgroup\" class=\"row-fluid-nowrap\">\n" +
    "                    <label ng-repeat=\"o in opt.label\" for=\"{{o.name}}\" class=\"checkbox span6\">\n" +
    "                        <input type=\"checkbox\" ng-model=\"o.checked\"  name=\"{{o.name}}\" id=\"{{o.name}}\" value=\"{{o.value}}\" />\n" +
    "                        <i class=\"skin\"></i>\n" +
    "                        <span>{{o.text}}</span>\n" +
    "                    </label>\n" +
    "                </div>   \n" +
    ...
    "        <button class=\"btn btn-alt btn-large\" ng-click=\"$dismiss('cancel')\">Cancel</button>\n" +
    "        <button class=\"btn btn-large\" ng-click=\"$close(options)\" >Apply</button>\n" +
    "    </div>\n" +
    "</div>");
}]);

angular.module("template/modal/u-filter.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/modal/u-filter.html",
    "<div>\n" +
    "    <a href=\"#\" class=\"btn-group\" role=\"button\">\n" +
    "        <span class=\"btn btn-utility\">Refine</span>\n" +
    "        <span class=\"btn btn-utility\"><i class=\"cssIcon-plus\"></i></span>\n" +
    "    </a>\n" +
    "</div>\n" +
    "");
}]);
...
angular.module("template/modal/window.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/modal/window.html",
    "<div tabindex=\"-1\" role=\"dialog\" att-element-focus=\"focusModalFlag\" class=\"modal fade {{ windowClass }}\" ng-class=\"{in: animate}\" ng-style=\"{'z-index': 1050 + index*10}\" ng-transclude></div>\n" +
    "");
}]);

angular.module("template/pagination/pagination.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/pagination/pagination.html",
    "<div><ul class=\"thumbnails\" aria-live=\"polite\" aria-relevant=\"additions\" aria-atomic=\"false\">\n" +
    "        <li ng-repeat=\"search in searchResult| limitTo:showQuantity\"><img tabindex=\"0\" ng-src=\"{{search.imageUrl}}\" alt=\"{{search.alt}}\"  class=\"img-bullet-src\">\n" +
    "            <div class=\"img-bullet-content\">\n" +
    "                <p>{{search.desc}}</p>\n" +
    "            </div></li>\n" +
    "    </ul><div>\n" +
    "        <button class=\"btn btn-large btn-block\" button-spinner=\"Loading...\">Show <em><span ng-bind=\"noOfResult\">\n" +
    "                </span></em> more</button></div>\n" +
    "</div>\n" +
    "");
}]);

angular.module("template/promos/promos.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/promos/promos.html",
    "<div att-carousel interval=\"interval\" no-pause=\"noPause\">\n" +
    "    <div att-carousel-content ng-repeat=\"image in attPromos.images\">\n" +
    "        <img  ng-src=\"{{image.Links[0].LinkGraphic}}\" width=\"100%\" alt=\"promos\">\n" +
    "    </div>\n" +
    "</div>");
}]);

angular.module("template/quantitySelect/quantity-select.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/quantitySelect/quantity-select.html",
    "<div class=\"btn-group btn-quantity-toggle\">\n" +
    "<div class=\"input-prepend input-append\">\n" +
    "    <button class=\"btn btn-prev pull-left\" ng-click=\"minus()\">\n" +
    "        <i class=\"cssIcon-minus\" style=\"background-image:none !important\">remove one</i>\n" +
    "    </button>\n" +
    "    <span ng-transclude></span>\n" +
    "    <button class=\"btn btn-next pull-left\" ng-click=\"plus()\">\n" +
    "        <i class=\"cssIcon-plus\" style=\"background-image:none !important\">add one</i>\n" +
    "    </button>\n" +
    "</div>\n" +
    "</div>");
}]);

angular.module("template/rotation/attCarousel.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/rotation/attCarousel.html",
    "<div ng-mouseenter=\"pause()\" ng-mouseleave=\"play()\" class=\"carousel\" hm-swipe-right=\"prev()\" hm-swipe-left=\"next()\">\n" +
    "    <div class=\"carousel-inner\" ng-transclude></div>\n" +
    "</div>\n" +
    "");
}]);

angular.module("template/rotation/attCarouselContent.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/rotation/attCarouselContent.html",
    "<div ng-class=\"{\n" +
    "    'active': leaving || (active && !entering),\n" +
    "    'prev\n" +
    "    ': (next || active) && direction=='prev',\n" +
    "    'next\n" +
    "    ': (next || active) && direction=='next',\n" +
    "    'right\n" +
    "    ': direction=='prev',\n" +
    "    'left\n" +
    "    ': direction=='next'\n" +
...
angular.module("template/stepIndicator/step-indicator.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/stepIndicator/step-indicator.html",
    "<div class=\"step-wrapper\" data-bubble-quantity=\"{{steps + 1}}\">\n" +
    "    <ol class=\"step-indicator\">\n" +
    "        <li indicator-spacer ng-repeat=\"number in numlength\" class=\"{{number.cls}}\">\n" +
    "            <span class=\"hidden-spoken\">{{number.text}} </span>\n" +
    "        </li>\n" +
    "    </ol>\n" +
    "    <h1 ng-if=\"stepName\" class=\"step-label\">\n" +
    "    	{{stepName}}\n" +
    "    	<div ng-if=\"stepDesc\" class=\"step-description\">{{stepDesc}}</div>\n" +
    "    </h1>\n" +
    "</div>");
}]);
...
angular.module("template/tabs/tab.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/tabs/tab.html",
    "<li ng-class=\"{active: active, disabled: disabled}\" aria-controls=\"threetab1\" class=\"tab\" tabindex=\"0\">\n" +
    "  <a href=\"javascript:void(0);\" ng-click=\"select()\" tab-heading-transclude><span ng-if=\"imgurl\"><img ng-src=\"{{imgurl}}\" alt=\"{{altText}}\"></span>{{heading}}</a>\n" +
    "</li>\n" +
    "");
}]);
...
angular.module("template/tabs/tabset-titles.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/tabs/tabset-titles.html",
    "<ul class=\"tabs promo-tabs\" ng-class=\"{'nav-stacked': vertical}\">\n" +
    "</ul>");
}]);
...
angular.module("template/toggle/button-toggle-spanish.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/toggle/button-toggle-spanish.html",
    "<div class=\"pull-right\">\n" +
    "    <label  class=\"btn btn-large btn-switch clearfix\" ng-transclude>\n" +
    "        \n" +
    "    </label>\n" +
    "</div>\n" +
    "\n" +
    ...
    "\n" +
    "\n" +
    " var buttonTemplate = '<span class=\"btn btn-slider-on\">{{onValue}}<span class=\"hidden-spoken\">{{onValue}}</span></span>' +\n" +
    "                                '<span class=\"btn btn-alt btn-slider\"></span>' +\n" +
    "                                '<span class=\"btn btn-slider-off pull-right\">{{offValue}}<span class=\"hidden-spoken\">{{offValue}}</span></span>';\n" +
    "                        \n" +
    "                        element.removeAttr('toggle-button');\n" +
    "                        element.removeAttr('ng-model');\n" +
    "\n" +
    "\n" +
    "<div class=\"pull-right\">\n" +
    "    <label  class=\"btn btn-switch clearfix pull-right\" ng-transclude>        						 \n" +
    "    </label>\n" +
    "</div>");
}]);

angular.module("template/toggle/button-toggle.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/toggle/button-toggle.html",
    "<div class=\"pull-right\">\n" +
    "    <label  class=\"btn btn-switch clearfix pull-right\" ng-transclude>        						 \n" +
    "    </label>\n" +
    "</div>\n" +
...
angular.module("template/tooltip/tooltip-html-unsafe-popup.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/tooltip/tooltip-html-unsafe-popup.html",
    ...
    "    <div class=\"helpertext\" style=\"display: block\" >\n" +
...
angular.module("template/tooltip/tooltip-popup-user.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/tooltip/tooltip-popup-user.html",
    "<div>\n" +
    "    <button type=\"button\" class=\"btn btn-alt btn-tooltip\" style=\"padding-top:16px\" title=\"Help\" ><i class=\"hidden-spoken\">Help</i></button>\n" +
    "    <div class=\"helpertext\" ng-transclude> </div>\n" +
    "</div>\n" +
    "");
}]);
...
angular.module("template/tooltip/tooltip-sandbox-style.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/tooltip/tooltip-sandbox-style.html",
    "<span class=\"tooltip\">\n" +
    "    <a href=\"#\" class=\"cssIcon-tooltip\" data-placement=\"{{tooltipPlacement}}\" role=\"button\">Tooltip<div class=\"arrow\"></div></a>\n" +
    "    <div class=\"tooltip-wrapper\"> \n" +
    "        <div class=\"helpertext\" role=\"tooltip\" ng-transclude ></div>\n" +
    "    </div>\n" +
    "</span>\n" +
    "");
}]);

angular.module("template/usagebar/bar.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/usagebar/bar.html",
    "<div class=\"bar\" ng-class='type && \"bar-\" + type'></div>");
}]);

angular.module("template/usagebar/usage.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/usagebar/usage.html",
